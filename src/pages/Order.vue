...
const hasOrdered = computed(() => {
    return productsToOrder.value.some(p => orderedItems[p.id]?.ordered)
})

const handleOrderSubmit = (): void => {
    productsToOrder.value.forEach(product => {
        const order = orderedItems[product.id]
        if (order?.ordered && order.quantity > 0) {
            updateProduct({
                ...product,
                actualAmount: product.actualAmount + order.quantity
            })
        }
    })
    ...
    <div class="page-container">
        ...
        <div v-if="productsToOrder.length > 0">
            <table>
                ...
                <tbody>
                    <tr v-for="product in productsToOrder" :key="product.id">
                        ...
                        <td><span class="status-warning"><strong>{{ product.minimumAmount - product.actualAmount }} </strong></span></td>
                        <td>
                            <input 
                                type="checkbox" 
                                :checked="orderedItems[product.id]?.ordered || false"
                                @change="(e) => {
                        ...
                        <td>
                            <input 
                                v-if="orderedItems[product.id]?.ordered"
                                type="number" 
                                min="0"
                                :value="orderedItems[product.id]?.quantity || 0"
                                @input="(e) => {
                                    if (!orderedItems[product.id]) {
            ...
            <div style="text-align: center; margin-top: 2rem;">
                <button v-if="hasOrdered" @click="handleOrderSubmit" class="submit-order-btn">Nieuwe Producten Besteld</button>
                <button v-if="hasOrdered" @click="$router.push('/')" class="cancel-submit-order-btn">Nog niet besteld</button>
            </div>
        </div>
